# https://github.com/VProv/BPE-Dropout.git
import heapq
import numpy as np
import sys
...
def process():
    ...
    outfile = sys.argv[2]
    bpecode = sys.argv[3]
    num_iter = int(sys.argv[4]) if len(sys.argv)>=5 else 5
    drop_rate = float(sys.argv[5]) if len(sys.argv)==6 else 0.1

    lines = read_file(infile)
    merge_table = load_subword_nmt_table(bpecode)

    subword_nmt_tokenizer = BpeOnlineTokenizer(
        bpe_dropout_rate=drop_rate,
...
if __name__ == '__main__':
    assert len(sys.argv) >= 4, f"usage: python {sys.argv[0]} <infile> <outfile> <bpecode> <num_iter=5>(opt) <drop_rate=0.1>(opt) "
    process()
